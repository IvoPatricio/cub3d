...
void	rotate_player_view(t_play *play, int rotate)
{
	double	old_dir_x;
	double	old_plane_x;
...
int	ft_keys(int key_code, t_map *map)
{
	...
	if (key_code == RIGHT)
		rotate_player_view(map->play, -ROTATE_SPEED);
	if (key_code == LEFT)
		rotate_player_view(map->play, ROTATE_SPEED);
	//raycaster_start(all);
...
void ft_mlx_init(t_map *map)
{
	...
	map.address = mlx_get_data_addr(map.img_ptr, &map.bits, &map.line_len, \
		&map.end);
	raycast(map);
	mlx_put_image_to_window(map.mlx_ptr, map.win_ptr, map.img_ptr, 0, 0);*/
	//mlx_do_key_autorepeaton(map.mlx_ptr);
	mlx_hook(map->win_ptr, 2, 1, ft_keys, map);
	mlx_hook(map->win_ptr, 33, (1L << 17), ft_destroyer, map); // Closing with esc
	mlx_hook(map->win_ptr, 17, 1L << 17, ft_destroyer, map); // closing with X
